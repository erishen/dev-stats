...
# 默认 GitHub 用户名：无 gh 登录 / GITHUB_TOKEN 时 dev-stats 必须显式 --user，
# 否则直接"未指定 --user，且未检测到登录态"退出。可用 make ... USER=其它 覆盖。
DEFAULT_USER ?= erishen
...
# 与 juejin/actions/ci 一致支持 ARGS 透传：容器/无 gh 登录时可用
# make csv ARGS="--user erishen"，或覆盖默认参数 make csv ARGS="--no-html"
csv: ## 导出仓库公开统计到 output/stats.csv（含详情/社区/活跃度，--no-traffic 不采流量所以快；不含 clone 列；同时生成 stats-preview.html）
	@mkdir -p output
	@uv run dev-stats --sort clones --detail --community --activity --no-traffic --csv output/stats.csv $(if $(ARGS),$(ARGS))

csv-traffic: ## 导出含 clone/views 流量的完整统计（= csv + 流量采集 + --include-traffic；clone/热度分析必须用这个）
	@mkdir -p output
	@uv run dev-stats --sort clones --detail --community --activity --include-traffic --csv output/stats.csv $(if $(ARGS),$(ARGS))
...
actions: ## 巡检各仓库 CI 状态（--actions，默认排除 fork 与老仓 CI 噪音，按最近推送排序，可 ARGS 覆盖）
	@uv run dev-stats --user $(DEFAULT_USER) --actions --no-forks --sort updated --exclude wildsKick,king-power,skeleton-ssr $(if $(ARGS),$(ARGS))

# 只巡检 CI：跳过流量采集与 97 行仓库大表，仅输出「CI 巡检小结」。
# 表格在窄终端/管道里会把 CI 列挤成 "-"，Agent 调用请用这个目标。
ci: ## 只输出 CI 巡检小结（跳过流量采集与仓库大表，输出短、速度快，适合 Agent/管道）
	@uv run dev-stats --user $(DEFAULT_USER) --ci-only --no-forks --sort updated --exclude wildsKick,king-power,skeleton-ssr $(if $(ARGS),$(ARGS))
